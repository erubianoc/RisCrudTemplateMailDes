...
using System.Text;
using TemplateMailRis.midleware;
...
namespace TemplateMailRis.controllers
{
    public class cnxCreateTempalte
    {
        ...
        private string tamplateContenDecode { get; set; }
        ...
        public int validaDuplicado()
        {
            ...
            using (var connection = new SqlConnection(StringConnectionDbris))
            {
                ...
                {
                    connection.Open();
                    string query = "SELECT count (id_template)  as existCampo " +
                        "from TBL_TEMPLATE_MAIL " +
                        "where CODIGO_CLIENTE = @cli AND CODIGO_PRODUCTO = @pr AND  id_service = @id_service";
                    using (SqlCommand cmd = new SqlCommand(query, connection))
                    {
                        cmd.CommandType = CommandType.Text;
                        cmd.CommandText = query;
                        cmd.Parameters.Add("@cli", SqlDbType.NVarChar);
                        cmd.Parameters["@cli"].Value = this.registro.client_code; ;
                        cmd.Parameters.Add("@pr", SqlDbType.NVarChar);
                        cmd.Parameters["@pr"].Value = this.registro.product_code;
                        ...
                        using (SqlDataReader reader = cmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                isDuplicado = Int16.Parse(reader["existCampo"].ToString());
                            }
                        }
                    }
                }
                catch (Exception e)
                {
                    ...
                    Console.WriteLine("ocurrio un erro consultando si el cleinte existe", e.Message);
                }
            }
            return isDuplicado;
        }
        ...
        public HttpResponseMessage insertTemplate()
        {
                ...
                using (var connection = new SqlConnection(StringConnectionDbris))
                {
                    try
                    {
                        ...
                        using (SqlCommand cmd = new SqlCommand(query, connection))
                        {
                            ...
                            cmd.Parameters["@template_value"].Value = this.tamplateContenDecode;
                    ...
                    catch (Exception e)
                    {
                        ...
                        {
                            Content = new StringContent("{\"message\" : el servidor no puede preocesar la solicitud" + e.Message + " }", Encoding.UTF8, "json/application")
                        };
            ...
            {
                respuesta = new HttpResponseMessage(HttpStatusCode.Conflict)
                {
                    Content = new StringContent("{" +
                      "\"message\" : \"registro esta duplicado en la db\"  }"
                      , Encoding.UTF8, "json/application")
                };
            }
            return respuesta;
